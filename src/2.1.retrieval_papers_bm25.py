#!/usr/bin/env python
# Based on BM25 for secondary screening of ArXiv metadata pool:
# 1. Read JSON generated by arxiv_fetch_raw.py (all papers);
# 2. Do BM25 indexing on title + abstract;
# 3. Use keywords / intent_queries in config.yaml as queries, calculate similarity;
# 4. Keep top_k papers for each query, and tag these papers (tag), a paper can have multiple tags;
# 5. Write the tagged paper list and top_k results for each query back to a new JSON file.
...
# This script is located in src/ and config.yaml is in the parent directory
...
@dataclass
class Paper:
  """Paper structure for BM25 retrieval phase (only care about metadata and tag)"""
  ...
  def text_for_bm25(self) -> str:
    """Text for BM25: title + abstract (with tags, for structured retrieval)"""
...
class BM25Index:
  """Lightweight BM25 implementation, avoiding extra dependencies."""
...
def load_config() -> dict:
  """
  Read config.yaml from repository root.
  Only build retrieval inputs based on intent_profiles, not compatible with legacy fields.
  """
  ...
  try:
    ...
    if isinstance(data, dict):
      return data
    log("[WARN] config.yaml top-level structure is not a dictionary, will ignore this config file.")
    return {}
...
def split_supabase_time_window(
  ...
  *,
  ...
  """
  Split longer time windows into multiple fixed-day chunks, to avoid statement timeout caused by single BM25 RPC.
  """
...
def merge_supabase_bm25_rows(
  ...
  *,
  ...
  """
  Merge multiple shard BM25 results:
  - Remove duplicates by id: when the same paper appears in multiple shards, keep the higher score;
  - Finally truncate to top_k by score.
  """
...
def query_supabase_bm25_with_shards(
  *,
  ...
  if not shards:
    return ([], "rpc shard query failed: no valid time shards generated")
  ...
  if success_count <= 0:
    ...
    return ([], f"rpc shard query failed: success=0/{len(shards)} | {detail}")

  summary = (
    f"rpc shard query success: {len(merged_rows)} papers"
...
def load_paper_pool(path: str) -> List[Paper]:
  ...
  for item in raw:
    try:
      ...
      log(f"[WARN] Failed to parse paper item, will skip: {e}")
...
def score_boolean_mixed_for_query(
  ...
  """
  BM25 boolean mixed mode:
  - AND/NOT: hard constraint (if not filtered, directly eliminated);
  - OR: soft enhancement when the same paper hits multiple branches.
  """
...
def rank_papers_for_queries(
  ...
  """
  Sort each query separately using BM25:
  - Tokenize query_text and BM25-score it against all papers;
  - Take the top top_k papers with the highest scores, record arxiv_id;
  - Tag these papers (tag), a paper can have multiple tags;
  ...
  """
  if not queries:
    log("[WARN] No queries parsed from config.yaml (intent_profiles), will return empty result.")
  ...
  for q in queries:
    ...
    log(f"[INFO] BM25 processing query ({q.get('type')}): tag={q.get('tag') or ''}")
    ...
    if scores is None:
      working_scores = bm25.score(tokenize(q_text))
      total_weight = 1.0
    else:
      working_scores = scores

    if total_weight > 0:
      working_scores = [s / total_weight for s in working_scores]
    candidate_indices = list(range(len(working_scores)))
    ...
    indices = sorted(candidate_indices, key=lambda i: working_scores[i], reverse=True)[:k]
    sim_scores: Dict[str, Dict[str, float | int]] = {}
...
def save_tagged_results(
  ...
  Write results to JSON:
  {
    ...
    "papers": [ { id, title, abstract, ..., tags: [...] }, ... ]  // Only keep papers with at least one tag
  }
  """
  ...
  log(f"[INFO] Tagged papers and top_k results for each query written to: {output_path}")
  log(f"[INFO] Number of tagged papers: {len(tagged_papers)}")


def main() -> None:
  parser = argparse.ArgumentParser(
    description="Step 2.1: Use BM25 for keyword / LLM query retrieval on ArXiv paper pool and tag the results.",
  )
  parser.add_argument(
    "--input",
    type=str,
    default=None,
    help="Optional: only process specified raw JSON file; if omitted, will batch process all .json files in archive/YYYYMMDD/raw directory.",
  )
  parser.add_argument(
    "--output",
    type=str,
    default=None,
    help="Optional: when using --input to process a single file, customize output JSON path; in batch mode, will automatically write to archive/YYYYMMDD/filtered directory, default suffix .bm25.json.",
  )
  parser.add_argument(
    "--top-k",
    type=int,
    default=None,
    help="Number of top K papers to keep for each query; if not specified, adapt based on total number of original papers: <=1000 papers take 50, 50 more for each additional 1000 papers.",
  )
  parser.add_argument(
    "--k1",
    type=float,
  ...
  parser.add_argument(
    ...
    action="store_true",
    help="Disable Supabase BM25 retrieval, force using local BM25 index.",
  )
  ...
  log(
    "[INFO] Supabase BM25 recall window:"
  ...
  if not queries:
    log("[ERROR] Unable to parse BM25 queries from subscription config, exiting.")
    return
  ...
  for source_key in query_groups:
    ...
    if not get_source_backend(config, source_key):
      log(f"[ERROR] Keyword references paper source \"{source_key}\", but source_backends.{source_key} is not configured.")
      return
  ...
  def run_supabase_rank_for_source(output_path: str, source_key: str, source_queries: List[dict]) -> dict | None:
    ...
    if not backend_enabled:
      if source_key == ARXIV_SOURCE_KEY:
        return None
      raise RuntimeError(f"Paper source \"{source_key}\" is not configured with a usable BM25 RPC.")
    ...
    if args.top_k is None or args.top_k <= 0:
      ...
      log(f"[INFO] Supabase BM25 window count (source={source_key}): {count_msg}")
      dynamic_top_k = estimate_dynamic_top_k(count_value)
      log(
        f"[INFO] Supabase BM25 adaptive Top K = {dynamic_top_k} "
        f"(source={source_key}, window_count={count_value if count_value is not None else 'unknown'}), "
        f"output file: {label}"
      )
    else:
      dynamic_top_k = args.top_k
      log(f"[INFO] Supabase BM25 using command line specified Top K = {dynamic_top_k}, source={source_key}, output file: {label}")
    ...
    try:
      ...
      if total_hits > 0:
        log(f"[INFO] Supabase BM25 hit {total_hits} papers (source={source_key}).")
      else:
        log(f"[WARN] Supabase BM25 missed (source={source_key}).")
      return result_sb
    except Exception as e:
      if source_key == ARXIV_SOURCE_KEY:
        log(f"[WARN] Supabase BM25 exception, will fallback to local BM25: {e}")
        return None
      raise
    finally:
      group_end()
  ...
  def run_multi_source_supabase_rank(output_path: str, source_queries: List[dict]) -> dict | None:
    ...
    log(f"[INFO] Multi-source BM25 window count: {count_msg}")
    ...
    try:
      ...
      total_hits = int(result_sb.get("total_hits") or 0)
      if total_hits > 0:
        log(f"[INFO] Multi-source BM25 hit {total_hits} papers.")
        return result_sb
      log("[WARN] Multi-source BM25 missed.")
      return None
    finally:
      group_end()
  ...
  def process_single_file(input_path: str, output_path: str) -> None:
    ...
    if need_local_arxiv:
      ...
      if not papers:
        log(f"[ERROR] Paper pool is empty, and arxiv query cannot be hit by Supabase: {input_path}")
      else:
        total_papers = len(papers)
        if args.top_k is None or args.top_k <= 0:
          dynamic_top_k = estimate_dynamic_top_k(total_papers)
          log(
            f"[INFO] File {os.path.basename(input_path)} has {total_papers} original papers, "
            f"adaptive set each query Top K = {dynamic_top_k}."
          )
        else:
          dynamic_top_k = args.top_k
          log(
            f"[INFO] File {os.path.basename(input_path)} using command line specified Top K = {dynamic_top_k}, "
            f"has {total_papers} original papers."
          )

        group_start(f"Step 2.1 - build BM25 index ({os.path.basename(input_path)})")
        log(f"[INFO] Building BM25 index for {total_papers} papers...")
    ...
    if not merged.get("queries"):
      log(f"[WARN] Current file has no BM25 results: {input_path}")
      return
  ...
  if args.input:
    ...
    if not raw_files:
      ...
      if merged_results:
        ...
        log("[WARN] No local original file, and no results returned by any source backend.")
      return

    log(f"[INFO] Batch mode: will process {len(raw_files)} JSON files in {RAW_DIR}.")
