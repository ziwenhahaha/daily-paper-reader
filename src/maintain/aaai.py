...
def main() -> None:
    parser = argparse.ArgumentParser(description="Maintenance entry: AAAI retrieval + Supabase sync.")
    ...
    cleanup_backend(backend_key="aaai", retention_days=args.retention_days, skip_cleanup=args.skip_cleanup)

    start_year = int(args.year_end) - int(args.year_count) + 1
    raw_path = str(args.raw_input or "").strip() or default_raw_path(
        f"aaai-papers-{start_year}-{int(args.year_end)}",
        run_date,
    )
    ...
    run_step("Maintain AAAI", init_cmd)
