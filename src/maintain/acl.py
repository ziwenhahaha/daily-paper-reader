...
def main() -> None:
    ...
    cleanup_backend(backend_key="acl", retention_days=args.retention_days, skip_cleanup=args.skip_cleanup)

    start_year = int(args.year_end) - int(args.year_count) + 1
    raw_path = str(args.raw_input or "").strip() or default_raw_path(
        f"acl-anthology-{start_year}-{int(args.year_end)}",
        run_date,
    )
    ...
    init_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "init_acl.py"),
        "--year-end",
    ...
    run_step("Maintain ACL", init_cmd)
