...
def load_config() -> dict:
    try:
        ...
        log(f"[WARN] Failed to load config.yaml: {exc}")
        return {}
...
def fetch_medrxiv_metadata(
    *,
    ...
    if len(windows) > 1:
        log(f"🗓️  [medRxiv Ingest] Splitting windows into {chunk_days} days/chunks: {len(windows)} segments")

    for idx, (window_start, window_end) in enumerate(windows, start=1):
        log(
            f"[medRxiv] Fetching window {idx}/{len(windows)}:"
        ...
        try:
            ...
            log(f"[WARN] medRxiv window fetch failed, skipping: {exc}")
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch medRxiv paper metadata.")
