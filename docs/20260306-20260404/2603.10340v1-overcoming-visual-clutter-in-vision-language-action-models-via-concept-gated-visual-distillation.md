...
title_zh: 通过概念门控视觉蒸馏克服视觉-语言-动作模型中的视觉杂乱
authors: "Sangmim Song, Sarath Kodagoda, Marc Carmichael, Karthick Thiyagarajan"
date: 2026-03-11
pdf: "https://arxiv.org/pdf/2603.10340v1"
tags: ["query:wam-vla"]
score: 10.0
evidence: 克服VLA模型在精确操纵中的视觉干扰
tldr: "针对视觉语言动作模型（VLA）在杂乱环境中因背景噪声导致性能下降的“精度-推理差距”问题，本文提出了一种无需训练且模型无关的推理框架CGVD。该方法通过指令解析和双层目标精炼识别操控目标，并利用傅里叶修复技术消除干扰物，生成保留几何信息的清晰观测。实验表明，CGVD在复杂杂乱场景下显著提升了机器人操控的成功率，从43.0%提升至77.5%，增强了VLA模型的鲁棒性。"
selection_source: fresh_fetch
motivation: VLA模型在杂乱背景下容易受到语义噪声干扰，导致几何定位失效，从而产生“精度-推理差距”。
method: 提出CGVD框架，通过指令解析、双层目标精炼及傅里叶修复技术，在推理阶段动态消除视觉干扰并保留关键空间几何信息。
result: "在高度杂乱的操控任务中，CGVD将成功率从基准的43.0%大幅提升至77.5%，有效防止了性能崩溃。"
conclusion: 推理阶段的视觉蒸馏是实现鲁棒机器人操控的关键，CGVD为解决VLA模型在复杂环境下的泛化问题提供了有效方案。
---

## 摘要
视觉-语言-动作（VLA）模型展示了令人印象深刻的零样本泛化能力，但在杂乱环境中经常面临“精度-推理差距”（Precision-Reasoning Gap）。这种失效是由背景引起的特征稀释驱动的，即高频语义噪声破坏了精确操作所需的几何接地（geometric grounding）。为了弥补这一差距，我们提出了概念门控视觉蒸馏（CGVD），这是一种无需训练且模型无关的推理框架，旨在稳定 VLA 策略。CGVD 通过将指令解析为安全集和干扰集，并利用结合了交叉验证与空间消歧的双层目标细化过程，显式地惩罚假阳性并隔离真实的操作目标。随后，我们通过基于傅里叶变换的修复技术处理场景，生成清晰的观测图像，在主动抑制语义干扰项的同时保留关键的空间几何和视觉本体感受。在高度杂乱的操作任务中进行的广泛评估表明，CGVD 能够防止性能崩溃。在具有密集语义干扰项的环境中，我们的方法显著优于最先进的基准模型，成功率从基准的 43.0% 提升至 77.5%。通过强制执行严格的属性遵循，CGVD 将推理时视觉蒸馏确立为在杂乱环境中实现鲁棒机器人操作的关键前提。
