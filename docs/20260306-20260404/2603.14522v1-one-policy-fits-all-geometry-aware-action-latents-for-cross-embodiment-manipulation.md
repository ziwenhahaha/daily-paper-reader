...
tags: ["query:wam-vla"]
score: 9.0
evidence: 跨机器人实体的操纵策略学习
tldr: 针对跨机器人本体操作中动作空间和结构差异大的挑战，本文提出One-Policy-Fits-All (OPFA)框架。该框架通过几何感知潜表征（GaLR）构建共享动作空间，并利用统一重定向解码器提取特定动作。OPFA支持多种末端执行器（如夹爪、灵巧手）的端到端联合训练，显著提升了数据效率和技能迁移能力，在11种本体实验中表现优异。
selection_source: fresh_fetch
motivation: 机器人本体间的结构和动作空间差异阻碍了多源数据的联合训练，限制了操作技能的规模化扩展。
method: 提出OPFA框架，利用3D卷积和Transformer构建几何感知潜表征，并通过统一解码器实现无需微调的动作重定向。
result: "在11种不同末端执行器上的实验显示，联合训练使成功率提升超50%，且仅需极少量样本即可实现新本体的快速迁移。"
conclusion: OPFA通过构建共享的几何感知动作空间，成功实现了跨本体的通用策略学习，大幅降低了机器人技能获取的成本。
---

## 摘要
跨具身操纵对于提升机器人操纵的可扩展性并降低高昂的数据采集成本至关重要。然而，具身之间的显著差异（例如动作空间的变化和结构上的不一致）给跨多源数据的联合训练带来了挑战。为了解决这一问题，我们提出了 One-Policy-Fits-All (OPFA) 框架，该框架能够跨多个具身学习单一且通用的策略。我们首先学习了一种几何感知潜表示 (GaLR)，它利用 3D 卷积网络和 Transformer 在不同具身之间构建了一个共享的潜动作空间。随后，我们设计了一个统一的潜重定向解码器，用于从潜表示中提取特定具身的动作，且无需进行任何特定具身的解码器微调。OPFA 实现了对来自不同具身（包括各种夹爪和具有任意自由度的灵巧手）数据的端到端协同训练，显著提高了数据效率并降低了技能迁移成本。我们在 11 种不同的末端执行器上进行了广泛的实验。结果表明，通过利用异构具身数据，OPFA 在多种设置下显著提升了策略性能。例如，与单源训练相比，跨具身协同训练可将成功率提高 50% 以上。此外，仅通过为新具身添加少量演示（例如 8 个），OPFA 即可达到与使用 72 个演示训练良好的模型相当的性能。
