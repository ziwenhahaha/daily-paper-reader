...
tags: ["query:wam-vla"]
score: 8.0
evidence: 视觉-语言-动作 (VLA) 模型的异常检测
tldr: 针对VLA模型在动态和分布外（OOD）环境下可靠性不足的问题，本文提出RC-NF，一种用于机器人操作实时异常检测的条件归一化流模型。该方法通过解耦机器人状态与物体运动轨迹，仅需正样本即可进行无监督训练，并在推理时提供精确的异常评分。实验表明，RC-NF在LIBERO-Anomaly-10基准上达到SOTA，且能作为即插即用模块集成到VLA系统中，实现毫秒级响应的异常干预，显著提升了系统的鲁棒性。
selection_source: fresh_fetch
motivation: 现有的VLA模型在面对动态环境和分布外情况时难以稳定运行，缺乏有效的实时监控和干预机制。
method: 提出RC-NF模型，利用归一化流解耦处理机器人与物体状态，通过概率密度函数在无监督条件下计算异常得分。
result: 在提出的LIBERO-Anomaly-10基准上取得SOTA性能，并在真实世界中实现了低于100ms的实时异常检测与干预。
conclusion: RC-NF作为一种高效的即插即用模块，能显著增强VLA机器人在复杂动态环境下的任务适应性和鲁棒性。
---
