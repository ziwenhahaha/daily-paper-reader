...
tags: ["query:wam-vla"]
score: 8.0
evidence: 将基础VLM适配为在线奖励生成器以优化机器人策略
...
motivation: 传统的机器人强化学习受限于难以设计出具有强泛化能力的奖励函数，阻碍了策略的在线优化。
method: 利用在大规模多源数据上训练的VLM，根据当前视觉观测实时生成由过程、完成和时间对比组成的复合奖励信号，对模仿学习策略进行闭环微调。
result: 在长程操控基准测试中，该方法在仅30次RL迭代内便显著提升了初始策略的成功率，且奖励模型表现出优秀的零样本泛化性能。
conclusion: VLM生成的奖励信号能有效替代人工奖励工程，为机器人学习提供可靠反馈，实现高效的在线策略优化。
---

## 摘要
强化学习 (RL) 在优化机器人操纵策略方面展现出巨大潜力，但其有效性仍受限于设计可泛化奖励函数的难度。在本文中，我们提出了一个在线策略优化框架，通过将基础视觉语言模型 (VLM) 适配为在线奖励生成器。我们基于最先进的 VLM 开发了一个鲁棒且可扩展的奖励模型，该模型在包含真实机器人轨迹、人机交互以及多样化模拟环境的大规模多源数据集上进行了训练。与以往事后评估整个轨迹的方法不同，我们的方法利用 VLM 根据当前的视觉观测构建多维奖励信号，包括过程奖励、完成奖励和时间对比奖励。我们以通过模仿学习 (IL) 训练的基础策略作为初始化，利用这些 VLM 奖励引导模型以闭环方式纠正次优行为。我们在具有挑战性的长程操纵基准测试上评估了我们的框架，这些任务需要顺序执行和精确控制。至关重要的是，我们的奖励模型在这些测试环境中以纯零样本 (zero-shot) 的方式运行。实验结果表明，我们的方法在仅 30 次 RL 迭代内就显著提高了初始 IL 策略的成功率，展现了卓越的样本效率。这一实证结果强调，VLM 生成的信号可以提供可靠的反馈以解决执行错误，从而有效地消除了对手动奖励工程的需求，并促进了机器人学习的高效在线优化。
