...
title_zh: ZeroWBC：直接从人类第一视角视频中学习自然的视觉运动人形机器人控制
...
tags: ["query:wam-vla"]
score: 9.0
evidence: 基于人类视频的人形机器人全身控制
tldr: ZeroWBC是一个无需机器人遥操作数据的人形机器人全身控制框架。它直接从人类第一视角视频中学习，通过微调视觉语言模型（VLM）根据指令和视觉上下文预测人体动作，再将其重定向至机器人关节并由鲁棒的跟踪策略执行。该方法在Unitree G1机器人上实现了比基准更自然、多样的交互行为（如坐下、踢球），为通用人形机器人控制提供了一种高效且可扩展的新范式。
selection_source: fresh_fetch
motivation: 传统人形机器人控制依赖昂贵的遥操作数据且动作僵硬，难以实现如坐下或踢球等自然且多样的场景交互。
method: 该框架首先利用微调后的VLM根据文本和第一视角视频预测未来人体动作，随后将动作重定向并利用通用的运动跟踪策略在机器人上执行。
result: 在Unitree G1机器人上的实验表明，该方法在动作自然度和任务多样性方面均优于基准方法，且无需任何机器人遥操作数据。
conclusion: ZeroWBC成功建立了一套从人类视频直接学习全身控制的流水线，为实现通用、自然的机器人场景交互提供了高效且可扩展的解决方案。
---

## 摘要
实现人形机器人在场景交互中多功能且自然的全身控制（WBC）仍然是一个重大挑战。尽管最近的一些工作展示了自主的人形机器人交互控制，但它们受限于僵化的运动模式和昂贵的遥操作数据采集，缺乏执行坐下或踢球等更具类人自然行为的多功能性。此外，获取必要的真实机器人遥操作数据极其昂贵且耗时。为了解决这些局限性，我们提出了 ZeroWBC，这是一个直接从人类第一视角视频中学习自然人形机器人视觉运动控制策略的新颖框架，消除了对大规模机器人遥操作数据的需求，并实现了自然的人形机器人场景交互控制。具体而言，我们的方法首先微调一个视觉语言模型（VLM），根据文本指令和第一视角视觉上下文预测未来的人类全身运动，然后将生成的运动重定向到真实的机器人关节，并通过我们鲁棒的通用运动跟踪策略执行，以实现人形机器人的全身控制。在宇树（Unitree）G1 人形机器人上进行的广泛实验表明，我们的方法在运动自然度和多功能性方面优于基准方法，成功建立了一个消除人形机器人全身控制中遥操作数据采集开销的流水线，为通用人形机器人全身控制提供了一种可扩展且高效的范式。
