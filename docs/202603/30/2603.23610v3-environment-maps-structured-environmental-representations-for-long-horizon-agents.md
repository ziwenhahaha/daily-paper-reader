...
tags: ["query:q2"]
score: 7.0
evidence: 包含参数化动作的结构化环境表示
tldr: 针对大语言模型在长程软件工作流中易出错且难以应对环境随机性的问题，本文提出了“环境地图（Environment Maps）”。这是一种持久化、与智能体无关的结构化图表示，通过整合屏幕录像和执行轨迹，将环境抽象为上下文、动作、工作流和默会知识。在WebArena基准测试中，该方法显著提升了任务成功率，为长程规划提供了可解释且可编辑的基础。
selection_source: fresh_fetch
motivation: 现有的智能体在复杂软件操作中常因环境动态变化和长程依赖导致级联错误，缺乏持久且结构化的环境认知。
method: 提出一种包含上下文、参数化动作、观测工作流和领域知识的结构化图表示法，将异构证据转化为可复用的环境地图。
result: "在WebArena测试中，配备环境地图的智能体成功率达到28.2%，远高于仅依赖会话上下文的基准（14.2%）及原始轨迹数据（23.3%）。"
conclusion: 环境地图为长程智能体提供了一个结构化、可解释且可增量优化的接口，有效增强了其在动态环境中的规划与执行能力。
---
