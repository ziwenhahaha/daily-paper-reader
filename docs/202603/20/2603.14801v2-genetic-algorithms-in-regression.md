...
date: 2026-03-16
pdf: "https://arxiv.org/pdf/2603.14801v2"
tags: ["query:llm-ec"]
score: 7.0
evidence: 用于离散优化问题的遗传算法
tldr: 针对统计学中离散参数空间优化难题（如非连续、非凸或搜索空间巨大），本文开发了GAReg遗传算法包。该工具通过紧凑的染色体表示和约束保留算子，有效解决了回归样条的最优节点放置和最佳子集变量选择等问题。在穷举法或动态规划失效的高维模型空间中，GAReg能高效提供近优解，为复杂回归优化提供了稳健的计算框架。
selection_source: fresh_fetch
motivation: 传统的梯度下降法在处理非连续、非凸或高维离散参数空间的回归优化问题时往往失效。
method: 提出GAReg遗传算法包，采用紧凑染色体表示、约束保留的交叉变异算子及可选的岛屿模型并行化技术。
result: 该方法能高效搜索高维模型空间，在穷举搜索或整数规划不可行的情况下提供接近最优的解决方案。
conclusion: GAReg为处理复杂的离散回归优化问题提供了一个统一且高效的遗传算法框架。
---
