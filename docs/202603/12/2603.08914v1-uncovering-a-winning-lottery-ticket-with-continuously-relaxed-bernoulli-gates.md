...
motivation: 现有的强彩票子网络发现方法依赖不可微的评分选择机制，限制了优化效率和在大规模模型上的扩展性。
method: 引入连续松弛的伯努利门控，通过全微分的端到端优化和L0正则化，在冻结权重的随机初始化网络中筛选子网络。
result: "在ResNet和Vision Transformer等模型上实现了高达90%的稀疏度，且在同等精度下稀疏能力接近主流edge-popup方法的两倍。"
conclusion: 该方法建立了首个无需直通估计器（STE）的全微分SLT发现框架，为预训练网络的稀疏化提供了高效且可扩展的路径。
---
