...
tldr: 现有基于图神经网络的子图模式检测方法通常假设图数据是集中存储的。当图分布在多方时，跨客户端的子图模式在本地变得不可识别，导致表示等价性差距。为此，本文提出一种逐步、逐层的嵌入交换框架，通过在前向传播中同步中间节点表示，在不泄露原始数据的前提下，有效恢复了与集中式模型一致的节点表示，弥补了大部分表示差距。
source: arxiv
...
motivation: 现有图神经网络方法在处理分布式图数据时，由于跨分区边界的子图模式在本地不可见，导致本地计算与集中式计算之间存在表示等价性差距。
method: 提出一种逐步、逐层的联邦嵌入交换框架，在每层前向传播中同步中间节点表示，且不暴露原始特征或标签。
result: 实验证明，嵌入交换与联邦参数聚合是互补的，只要每步交换的嵌入保持最新，两者的结合就能恢复大部分的表示差距。
conclusion: 该框架在保护隐私的同时，成功使分布式图计算逼近集中式模型的性能，有效解决了跨客户端子图模式检测的难题。
---

## 摘要
子图模式检测旨在揭示图中复杂的交互结构。然而，基于图神经网络（GNN）的现有先进解决方案假设能够对整个图进行集中式访问。当图分布于多方时，客户端本地的GNN计算会与集中式模型产生偏离，从而导致表示等价性差距。我们将其形式化为一个结构可观测性问题，在此问题中，跨越划分边界的子图模式在本地变得不可识别。为了弥合这一差距，我们提出了一个逐步、逐层的嵌入交换框架，在该框架中，客户端在前向传播的每一层同步中间节点表示，而无需暴露原始特征或标签。在扩展子图假设以及客户端间共享模型参数的条件下，该框架能够恢复与在全图上运行的集中式GNN相同的节点表示。在包含环、二分团和散射-聚集模式的合成有向多重图上的实验表明，嵌入交换与联邦参数聚合是互补的而非可互换的：只要交换的嵌入是每步更新的而非每轮陈旧的，两者的结合就能恢复大部分的表示差距。
