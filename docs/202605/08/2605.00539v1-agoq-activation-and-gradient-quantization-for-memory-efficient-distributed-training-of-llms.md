---
title: "AGoQ: Activation and Gradient Quantization for Memory-Efficient Distributed Training of LLMs"
title_zh: AGoQ：面向LLM内存高效分布式训练的激活与梯度量化
authors: "Wenxiang Lin, Juntao Huang, Luhan Zhang, Laili Li, Xiang Bao, Mengyang Zhang, Bing Wang, Shaohuai Shi"
date: 2026-05-01
pdf: "https://arxiv.org/pdf/2605.00539v1"
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 8位梯度量化与精度保持All-Reduce实现通信高效分布式训练
tldr: 大语言模型（LLM）的训练对GPU内存需求极高，现有的量化方法在处理4位激活值和8位梯度时容易导致收敛缓慢或精度下降。为此，本文提出了AGoQ框架，通过引入层感知激活量化算法和保持精度的8位梯度All-Reduce通信技术，在近乎无损的情况下大幅降低了显存占用并提升了训练速度。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2605.00539v1/fig-001.webp\", \"caption\": \"Figure 4. The forward and backward passes of attention and MLP for kernel fusion of quantization/dequantization and GEMM.\", \"page\": 6, \"index\": 1, \"width\": 350, \"height\": 412}, {\"url\": \"assets/figures/arxiv/2605.00539v1/fig-002.webp\", \"caption\": \"Figure 2. The integration of activation and gradient quantization with Megatron-LM.\", \"page\": 3, \"index\": 2, \"width\": 483, \"height\": 371}]"
motivation: 现有的LLM训练量化方法在处理4位激活值和8位梯度时效果不佳，容易引发收敛缓慢或精度丢失的问题。
method: AGoQ结合了层感知激活量化算法（根据层类型和流水线阶段分配位宽以实现近4位存储）以及保持精度的8位梯度All-Reduce通信技术。
result: "在多达64个GPU的集群上对8B至32B的LLaMA模型进行实验，AGoQ相比现有最先进系统最高减少了52%的内存，并实现了最高1.34倍的训练加速。"
conclusion: AGoQ能够在保持预训练收敛性能和下游任务精度的同时，显著降低大语言模型分布式训练的内存消耗并提升训练速度。
---

## 摘要
量化是降低大语言模型（LLMs）训练GPU内存需求的关键方法。然而，当前的方法在处理4位激活和8位梯度时效果不佳，容易导致收敛缓慢或精度损失。为了解决这一问题，我们提出了AGoQ，其中包含两项新技术：1）一种层感知的激活量化算法，该算法根据各层的类型和流水线阶段为其激活分配适当的位宽，从而实现接近4位的激活存储；2）一种梯度量化算法，通过采用8位梯度存储和保精度的8位All-Reduce通信，来降低内存使用并缩短通信时间。我们在两个GPU集群（最多64个GPU）上使用不同规模的LLMs进行了大量实验。实验结果表明，与最先进的训练系统Megatron-LM（带有或不带有ZeRO）、COAT和DeepSpeed在8B至32B LLaMA模型上的表现相比，我们的AGoQ最多可减少52%的内存，并实现高达1.34倍的训练速度提升，同时在预训练中保持相同的收敛损失，并在基于LLaMA架构的下游任务中取得可比的准确率。

## Abstract
Quantization is a key method for reducing the GPU memory requirement of training large language models (LLMs). Yet, current approaches are ineffective for 4-bit activations and 8-bit gradients, which would easily cause slow convergence or accuracy loss. To address this, we introduce AGoQ, incorporating two new techniques: 1) a layer-aware activation quantization algorithm that allocates appropriate bit-widths for activations of various layers based on their types and pipeline stages to achieve near 4-bit activation storage, and 2) a gradient quantization algorithm that reduces memory usage and shortens communication time by employing 8-bit gradient storage and precision-preserving 8-bit All-Reduce communication. We conduct extensive experiments using different sizes of LLMs on two GPU clusters (up to 64 GPUs), and the experimental results show that our AGoQ reduces the memory by up to 52\% and achieves up to 1.34$\times$ improvement of training speed compared to state-of-the-art training systems Megatron-LM (w/ or w/o ZeRO), COAT and DeepSpeed with 8B to 32B LLaMA models, while achieving convergence loss on pretraining and comparable accuracy on downstream tasks with LLaMA architectures.