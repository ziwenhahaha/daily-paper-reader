...
## 摘要
传统智能体框架以语言模型为核心，将日志作为事后观察层。本研究提出ActiveGraph，通过将append-only事件日志作为唯一真相来源，工作图作为日志的确定性投影，行为组件响应图变化并发出新事件。这一设计反转实现了：任意运行的可确定性重放、任意事件点的零开销分叉、以及从高层目标到单个模型调用的端到端因果追踪。以法律尽职调查为例验证了完整因果结构的可重建性，并探讨了其在自我改进智能体中的潜力。
