...
tags: ["query:dgnn-nas"]
score: 7.0
evidence: 通过稀疏图剪枝实现通信高效的分布式学习
tldr: 本文针对网络内学习（INL）中分布式模块通信开销大的问题，提出了D-INL方法。该方法通过保留基于容量感知的最短路径树来修剪非树链路，并将局部路由建模为有限速率的随机门以平衡稀疏性与预测信息。实验表明，该方法在保持模型精度的同时，大幅减少了训练通信交换量，有效提升了分布式学习的效率。
source: arxiv
selection_source: fresh_fetch
motivation: 为了解决网络内学习（INL）中分布式模块间交换激活值和误差带来的高昂通信开销问题。
method: 提出D-INL算法，利用容量感知的最短路径树修剪非树链路，并将局部路由建模为有限速率的随机门以优化稀疏性与信息量的平衡。
result: "在分布式分类实验中，D-INL减少了70.4%的训练通信交换量，且准确率与稠密INL保持一致；引入有限速率正则化后，估计的潜在速率进一步降低了45.7%。"
conclusion: 该方法能够在不牺牲模型准确率的前提下，显著降低网络内学习的通信开销和潜在传输速率。
---

## 摘要
网内学习（INL）通过在通信图上交换潜在激活值和反向传播误差来训练分布式神经模块。本文提出了Dijkstra剪枝INL（D-INL），该方法通过保留以融合节点为根的容量感知最短路径树来移除非树链路。为了平衡稀疏性与预测信息，将局部路由（或聚合）建模为速率为$R_g=I(Z; T)$的有限速率随机门。我们推导了速率-失真-泛化界，并在可复现的分布式分类实验上验证了该方法，其中D-INL在将训练交换量减少70.4%的同时，将准确率保持在密集INL的标准差范围内。与未正则化的Dijkstra INL相比，加入有限速率正则化进一步使估计的潜在速率降低了45.7%。
