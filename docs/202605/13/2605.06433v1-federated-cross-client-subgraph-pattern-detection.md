...
tldr: 现有的图神经网络（GNN）在集中式图上进行子图模式检测效果良好，但在图数据分布在多方时，由于存在表示等价性差距，跨客户端的子图模式会变得局部不可识别。为此，本文提出了一种逐步、逐层的嵌入交换框架，客户端在前向传播的每一层同步中间节点表示，且不暴露原始特征或标签。该框架有效弥合了表示差距，恢复了与集中式GNN相同的节点表示，主要贡献在于证明了嵌入交换与联邦参数聚合是互补的，两者的结合能最大程度恢复表示等价性。
source: arxiv
...
motivation: 现有基于GNN的子图模式检测方法依赖集中式图数据，当图分布在多方时，局部计算会产生表示等价性差距，导致跨客户端的子图模式无法被识别。
method: 提出一种逐步、逐层的嵌入交换框架，允许客户端在每层前向传播中同步中间节点表示，在不泄露原始数据的前提下恢复与集中式模型一致的节点表示。
result: 在包含环、二分图和散射-聚集模式的合成有向多重图上的实验表明，嵌入交换与联邦参数聚合是互补而非可互换的，两者的结合能恢复大部分表示差距。
conclusion: 只要交换的嵌入在每一步保持最新而非每个轮次使用陈旧数据，结合联邦参数聚合与嵌入交换即可有效弥合集中式与分布式GNN之间的表示等价性差距。
---
