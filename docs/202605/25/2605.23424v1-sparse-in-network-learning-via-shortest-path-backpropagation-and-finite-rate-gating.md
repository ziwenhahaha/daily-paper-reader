...
tags: ["query:dgnn-nas"]
score: 7.0
evidence: "通过最短路径剪枝减少分布式神经网络训练中70.4%的通信量"
tldr: 在分布式网络内学习（INL）中，模块间通信开销巨大。本文提出D-INL方法，通过保留容量感知的最短路径树修剪非树链路，并将局部路由建模为有限速率随机门。该方法在保持模型精度的同时，大幅减少了训练通信交换量，有效平衡了网络稀疏性与预测信息。
source: arxiv
selection_source: fresh_fetch
motivation: 为了降低分布式网络内学习中节点间交换激活值和误差的巨大通信开销。
method: 提出Dijkstra修剪的INL（D-INL）算法，利用容量感知最短路径树修剪链路，并将局部路由建模为有限速率的随机门以优化信息传输。
result: "实验表明，D-INL减少了70.4%的训练通信交换量，且准确率与稠密INL相当；加入有限速率正则化后，进一步降低了45.7%的潜变量传输速率。"
conclusion: 该方法能够在不牺牲分类准确率的前提下，显著提升分布式网络内学习的通信效率并有效降低传输速率。
---

## 摘要
网内学习（INL）通过在通信图上交换潜在激活值和反向传播误差来训练分布式神经模块。本文提出了Dijkstra剪枝INL（D-INL），该方法通过保留以融合节点为根的容量感知最短路径树来移除非树链路。为了平衡稀疏性与预测信息，将局部路由（或聚合）建模为速率为 $R_g=I(Z; T)$ 的有限速率随机门。我们推导了速率-失真-泛化界，并在可复现的分布式分类实验上验证了该方法，其中D-INL在将训练交换量减少70.4%的同时，将准确率保持在稠密INL标准差的范围内。加入有限速率正则化后，与未正则化的Dijkstra INL相比，估计的潜在速率进一步降低了45.7%。
