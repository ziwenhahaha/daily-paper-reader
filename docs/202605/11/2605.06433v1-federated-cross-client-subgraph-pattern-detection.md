...
tldr: 现有基于图神经网络的子图模式检测方法通常假设图数据是集中存储的。当图分布在多方时，跨客户端的子图模式在本地变得不可识别，导致表示等价性差距。为此，本文提出一种逐步、逐层的嵌入交换框架，通过在前向传播中同步中间节点表示，在不暴露原始数据的前提下，有效恢复了与集中式模型一致的表示能力，弥补了大部分表示差距。
source: arxiv
...
motivation: 现有图神经网络方法在处理分布式图数据时，存在跨分区子图模式局部不可识别的表示等价性差距问题。
method: 提出一种逐步、逐层的联邦嵌入交换框架，在保护隐私的前提下于前向传播中同步中间节点表示。
result: 实验证明，嵌入交换与联邦参数聚合是互补的，且使用每步最新的嵌入能最大程度恢复表示等价性差距。
conclusion: 结合每步更新的嵌入交换与联邦参数聚合，能够有效且互补地解决分布式图计算中的结构可观察性问题。
---
