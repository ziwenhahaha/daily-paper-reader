...
## 摘要
现有联邦强化学习方法多忽视环境异构性或仅能训练单一共享策略，无法实现协作且个性化的策略训练。本文提出单时尺度联邦演员-评论家框架，通过共享线性子空间表示结合局部个性化策略组件，在单一时尺度更新下利用迭代估计实现有限样本收敛，并在异构马尔可夫轨迹下建立critic误差和策略梯度收敛速率，实验验证于联邦Hopper-v5任务。
