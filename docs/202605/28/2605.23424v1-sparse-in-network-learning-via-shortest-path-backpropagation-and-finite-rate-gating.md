...
tags: ["query:dgnn-nas"]
score: 7.0
evidence: "通过最短路径反向传播稀疏化减少70.4%训练通信"
...
method: 提出D-INL算法，利用基于容量的最短路径树修剪非树链路，并将局部路由建模为有限速率的随机门以优化稀疏性与信息量的平衡。
result: "实验表明，D-INL在保持与稠密INL相当准确率的同时减少了70.4%的训练通信交换，加入有限速率正则化后进一步使潜在速率降低了45.7%。"
conclusion: 该方法能够在不牺牲模型预测性能的前提下，显著提升分布式网络内学习的通信效率。
---

## 摘要
网内学习（INL）通过在通信图上交换潜在激活值和反向传播误差来训练分布式神经模块。本文提出了Dijkstra剪枝INL（D-INL），该方法通过保留以融合节点为根的容量感知最短路径树来移除非树链路。为了平衡稀疏性与预测信息，将局部路由（或聚合）建模为速率为$R_g=I(Z; T)$的有限速率随机门。我们推导了速率-失真-泛化界，并在可复现的分布式分类实验上验证了该方法。在该实验中，D-INL减少了70.4%的训练交换量，同时将准确率保持在密集INL的标准差范围内。加入有限速率正则化后，与未正则化的Dijkstra INL相比，估计的潜在速率进一步降低了45.7%。
