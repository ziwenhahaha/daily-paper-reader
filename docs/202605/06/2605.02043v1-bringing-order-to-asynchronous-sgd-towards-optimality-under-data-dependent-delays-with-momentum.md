...
tldr: 异步随机梯度下降（ASGD）在分布式训练中常受梯度延迟影响，现有缓解策略通常会丢弃或衰减延迟梯度，导致简单样本的更新被过度代表。本文提出了一种基于动量的异步框架，旨在保留延迟梯度信息的同时减轻延迟影响，并在凸和非凸平滑设定下首次确立了数据依赖延迟的最优收敛率，同时推导出简化超参数调优的鲁棒学习率调度方案。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的异步SGD缓解策略通常会衰减或丢弃延迟梯度，导致复杂样本的梯度被抑制，从而引入系统性偏差。
method: 提出一种基于动量的异步优化框架，在减轻梯度延迟影响的同时保留延迟梯度中的信息。
result: 在凸和非凸平滑设定下，首次确立了数据依赖延迟的最优收敛率，并推导出简化超参数调优的鲁棒学习率调度。
conclusion: 该研究在标准假设下为异步优化提供了新结论，有效解决了数据依赖延迟问题，并简化了实际应用中的超参数调整。
---

## 摘要
异步随机梯度下降（SGD）实现了可扩展的分布式训练，但受到梯度延迟的困扰。现有的缓解策略，如延迟自适应学习率和延迟感知过滤，通常会衰减或丢弃延迟的梯度，从而引入系统性偏差：来自较简单或处理速度较快样本的更新被过度代表，而来自较复杂样本的梯度则被延迟或抑制。相比之下，先前处理数据依赖延迟的方法依赖于 Lipschitz 假设，导致收敛速率次优，或者未解决平滑凸的情况。我们提出了一个基于动量的异步框架，旨在保留延迟梯度中的信息，同时减轻延迟效应。我们在凸和非凸平滑设定下，为数据依赖延迟建立了首个最优收敛速率，为标准假设下的异步优化提供了新结果。此外，我们推导了鲁棒的学习率调度策略，简化了实践中的超参数调优。
