...
tldr: 异步随机梯度下降（SGD）在分布式训练中常受梯度延迟影响，现有缓解策略通常会衰减或丢弃延迟梯度，导致系统性偏差。本文提出一种基于动量的异步框架，在减轻延迟影响的同时保留延迟梯度信息，首次在数据依赖延迟下实现了最优收敛率，并推导出简化调参的鲁棒学习率方案。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的异步SGD缓解策略会衰减或丢弃延迟梯度，导致处理较慢的复杂样本信息被抑制，从而引入系统性偏差。
method: 提出一种基于动量的异步优化框架，旨在保留延迟梯度信息的同时有效减轻陈旧性的负面影响。
result: 在标准假设下，首次在平滑凸和非凸环境中为数据依赖延迟建立了最优收敛率，并推导出鲁棒的学习率调度方案。
conclusion: 该框架成功解决了异步优化中的数据依赖延迟问题，在保证理论最优性的同时，简化了实际应用中的超参数调优。
---
