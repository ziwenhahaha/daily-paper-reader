...
title_zh: Quality Audio Prototyping（QuAP）：一种统一声音检索与程序化生成的原型系统
...
score: 6.0
evidence: 程序化音频内容生成
tldr: 原型化统一系统用于音频设计中的声音检索和程序化生成。
source: arxiv
...
## 摘要
声音设计工作流经常在耗时的素材库搜索与程序化合成的复杂性之间摇摆，实践者通常依赖相互脱节的工具分别应对各自挑战。本文提出了 Quality Audio Prototyping（QuAP），一个工作原型系统，它在单一界面中统一了基于内容的音频检索与程序化声音生成，缩短了叙事概念与其声音实现之间的程序化距离。QuAP 将基于相似度的检索引擎与实时程序化音频模型相结合，并辅以基于规则的助理，提供基于感知的参数指导，其定义和建议源自经验优化，而无需预先掌握合成知识。初步评估证实了该方法的可行性：主观评估显示在六个嵌入合成模型中有五个在质量上取得了统计显著的提升，编码器消融研究确定了在音效数据集上首选的检索架构。针对 16 名从业者的用户评估证实了该工具在工作流中的实用性，所有参与者均认同参数助理保留了创作主动性，同时降低了程序化交互的门槛。
