...
tldr: "随着大规模机器学习模型不断增大，多GPU分布式训练中的通信开销成为严重瓶颈。本研究提出了一种资源感知的计算与通信重叠方法，通过调节计算内核的共享内存分配以控制其资源占用，并为通信内核分配更高的调度优先级，从而实现两者的并发执行。该方法无需修改底层库或内核代码，在多种GPU上最高可将总执行时间减少25.5%。"
source: arxiv
...
method: 通过控制计算内核的共享内存分配来限制其资源占用，并提升通信流的调度优先级，实现计算与通信的高效并发重叠。
result: "在NVIDIA和AMD多款GPU上的实验表明，该方法无需修改任何底层库即可将总执行时间最高降低25.5%。"
conclusion: 该运行时控制方法能够有效解决多GPU训练中的通信瓶颈，显著提升系统整体性能。
---
