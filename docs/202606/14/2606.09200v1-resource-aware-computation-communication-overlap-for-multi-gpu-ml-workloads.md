...
tldr: "多GPU分布式训练中，计算与通信的串行执行导致通信开销成为核心瓶颈。本文提出一种资源感知的计算与通信重叠机制，通过共享内存分配调节计算内核占用率，并提升通信流的调度优先级，实现两者的高效并发。该方法无需修改底层库即可将总执行时间降低高达25.5%。"
source: arxiv
...
motivation: 随着机器学习模型规模扩大，多GPU训练中的通信开销已成为影响性能的主要瓶颈。
method: 利用共享内存驱动的计算内核占用率调节和提升通信内核调度优先级这两种轻量级运行时控制，实现计算与通信的并发执行。
result: "在NVIDIA和AMD多款主流GPU上的实验表明，该方法成功实现了计算与通信的有效重叠，总执行时间最高减少了25.5%。"
conclusion: 该方法无需修改厂商库或内核代码，即可作为一种通用且高效的方案，显著优化多GPU机器学习工作负载的执行效率。
---
