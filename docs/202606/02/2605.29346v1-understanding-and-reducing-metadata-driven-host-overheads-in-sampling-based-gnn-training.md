...
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 减少主机开销以实现高效GNN训练的系统
tldr: 在基于采样的图神经网络（GNN）训练中，动态元数据驱动的执行机制导致CPU成为性能瓶颈。现有方案无法解决此问题，为此本文提出ZEROGNN系统。该系统将元数据保留在GPU端，在固定启动结构中实现完全驻留GPU的动态执行，消除了主机开销，实现了最高5.28倍的端到端加速。
source: arxiv
...
motivation: 基于采样的GNN训练中，动态元数据驱动的执行导致严重的CPU开销和频繁的GPU-CPU同步，而现有方法无法解决这种主机介导的控制循环问题。
method: ZEROGNN将运行时元数据保留在设备端，在固定的启动结构中协调动态执行，并提供紧凑的执行包络以恢复CUDA Graph的可重放性。
result: "实验表明，ZEROGNN在采样GNN任务上实现了最高5.28倍的端到端加速，GPU执行占比接近100%，并具备出色的多GPU扩展能力。"
conclusion: 通过将主机移出元数据驱动的控制循环，ZEROGNN成功消除了主机端瓶颈，显著提升了动态GNN训练的效率与性能。
---

## 摘要
现代深度学习工作负载日益表现出动态的、由元数据驱动的执行特征，即由运行时生成的信息来决定内存分配与内核启动决策。在基于采样的图神经网络（GNN）训练中，这种行为使得CPU成为关键路径，引入了持续的主机-设备协调开销以及频繁的GPU-CPU同步；当GPU计算量较小时，这些开销将主导端到端的运行时间。现有的方法（包括CUDA Graphs和GPU动态并行性）未能解决这一问题，因为由元数据驱动的控制循环仍由主机主导，且执行结构在不同迭代间存在差异。我们提出了ZEROGNN系统，该系统将主机从由元数据驱动的控制循环中移除，并支持在动态行为下实现完全常驻GPU的执行。ZEROGNN将运行时元数据保留在设备端，在固定的启动结构中协调动态执行，并提供保守但紧凑的执行包络，从而恢复CUDA Graph的可重放性。在基于采样的GNN工作负载上的实验表明，ZEROGNN实现了高达5.28倍的端到端加速，接近100%的GPU执行占比，以及与理想元数据感知分配相当的内存效率；同时，通过消除主机端瓶颈，实现了强大的多GPU扩展能力。
