...
tldr: 本文研究了图结构下且通信消息被压缩的去中心化在线凸优化问题。传统方法通常需要依赖问题参数调整学习率，且压缩通信会引入额外的节点分歧。为此，本文提出了一种名为DECO-EF的去中心化无参数在线学习算法，结合了掷币预测与基于差值的压缩Gossip通信机制。该算法无需预先设定时间跨度、比较器范数或学习率等参数，并首次在压缩通信下实现了无参数去中心化在线学习的期望次线性网络遗憾界保证。
source: arxiv
...
motivation: 传统去中心化在线学习方法需要依赖未知参数调整学习率，且通信压缩会加剧网络节点间的分歧，亟需一种无需调参且能克服通信瓶颈的算法。
method: 提出DECO-EF算法，将掷币预测与基于差值的压缩Gossip通信相结合，各节点仅传输压缩后的状态差值并利用误差反馈机制维持全局协同。
result: 在压缩通信环境下，理论上为DECO-EF算法证明了期望比较器自适应的网络遗憾界。
conclusion: 该研究首次在压缩通信条件下，为无参数去中心化在线学习提供了期望次线性网络遗憾的理论保证。
---

## 摘要
我们研究了智能体在图上进行通信且消息可能被压缩时的去中心化在线凸优化问题。经典的去中心化在线方法通常需要依赖于时间跨度、比较器尺度或其他问题参数的学习率选择，而压缩通信则会引入必须加以控制的额外分歧。我们提出了DECO-EF（带有误差反馈的去中心化抛币预测，DEcentralized COin-betting with Error Feedback），这是一种去中心化的无参数在线学习算法，它将抛币预测与基于差值的压缩Gossip相结合。每个智能体维护一个干净的累积状态和一个压缩跟踪器，并在Gossip步骤中仅通信压缩的状态差值。该方法在在线学习意义上是无参数的：它不需要针对时间跨度、比较器范数或学习率进行调整。我们证明了在压缩通信下DECO-EF的期望比较器自适应网络遗憾界。据我们所知，这为压缩通信下的无参数去中心化在线学习提供了首个期望次线性网络遗憾保证。
