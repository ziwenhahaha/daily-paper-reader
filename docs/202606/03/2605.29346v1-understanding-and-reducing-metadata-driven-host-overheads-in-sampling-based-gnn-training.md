...
tags: ["query:dgnn-nas"]
score: 7.0
evidence: GNN训练系统减少主机开销以实现高效执行
tldr: 在基于采样的图神经网络（GNN）训练中，元数据驱动的动态执行导致CPU成为性能瓶颈。现有方案无法解决该问题，因此本文提出ZEROGNN系统。它将元数据保留在GPU端，在固定结构中调解动态执行，从而将主机移出控制循环，实现了完全驻留在GPU上的执行。实验表明，该系统最高可实现5.28倍的端到端加速。
source: arxiv
...
motivation: 基于采样的GNN训练中，元数据驱动的执行导致严重的CPU开销和频繁的GPU-CPU同步，而现有方法无法解决这种动态控制循环带来的性能瓶颈。
method: ZEROGNN系统将运行时元数据保留在设备端，在固定的启动结构中调解动态执行，并提供紧凑的执行包络以恢复CUDA Graph的可重放性。
result: "在采样型GNN负载的实验中，ZEROGNN实现了高达5.28倍的端到端加速，GPU执行率接近100%，并具备出色的多GPU扩展能力。"
conclusion: 通过将主机移出元数据驱动的控制循环，ZEROGNN成功消除了宿主机端的性能瓶颈，显著提升了GNN训练的效率与扩展性。
---

## 摘要
现代深度学习工作负载日益表现出动态的、由元数据驱动的执行特征，即由运行时生成的信息来决定内存分配与内核启动决策。在基于采样的图神经网络（GNN）训练中，这种行为使得CPU成为关键路径，引入了持续的主机-设备协调开销以及频繁的GPU-CPU同步；当GPU计算量较小时，这些开销将主导端到端的运行时间。现有的方法（包括CUDA Graphs和GPU动态并行）未能解决这一问题，因为由元数据驱动的控制循环仍由主机主导，且执行结构在不同迭代间存在差异。我们提出了ZEROGNN系统，该系统将主机从由元数据驱动的控制循环中移除，并支持在动态行为下实现完全常驻GPU的执行。ZEROGNN将运行时元数据保留在设备端，在固定的启动结构中协调动态执行，并提供一个保守但紧凑的执行包络，从而恢复CUDA Graph的可重放性。在基于采样的GNN工作负载上的实验表明，ZEROGNN实现了高达5.28倍的端到端加速，接近100%的GPU执行占比，以及可与理想元数据感知分配相媲美的内存效率；同时，通过消除主机端瓶颈，实现了强大的多GPU扩展能力。
