...
## 摘要
动力学模型，无论是模拟器还是学习到的世界模型，长期以来一直是机器人操纵的核心，但大多数模型都专注于最小化预测误差，而不是面对一个更根本的挑战：现实世界的操纵本质上是不确定的。我们认为，不确定性下的鲁棒操纵从根本上说是一个集成问题：不确定性必须在规划循环中进行表示、传播和约束，而不仅仅是在训练期间被抑制。我们提出并开源了 ManiDreams，这是一个基于直觉物理模型进行不确定性感知操纵规划的模块化框架。它通过可组合的抽象实现了这种集成，包括分布状态表示、后端无关的动力学预测以及用于动作优化的声明式约束规范。该框架明确解决了三种不确定性来源：感知、参数和结构。它通过一个“采样-预测-约束”循环封装了任何基础策略，该循环根据分布结果评估候选动作，在无需重新训练的情况下增加了鲁棒性。在 ManiSkill 任务上的实验表明，在强化学习（RL）基准性能显著下降的各种扰动下，ManiDreams 仍能保持鲁棒的性能。在推、取、接以及现实世界部署中的可运行示例展示了其在不同策略、优化器、物理后端和执行器之间的灵活性。该框架可在 https://github.com/Rice-RobotPI-Lab/ManiDreams 公开获取。
