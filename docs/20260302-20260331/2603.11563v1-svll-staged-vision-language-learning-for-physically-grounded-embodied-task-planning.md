...
tags: ["query:wam-vla"]
score: 9.0
evidence: 用于物理落地的具身任务规划的分阶段视觉语言学习
tldr: 针对具身任务规划中视觉定位与因果一致性的挑战，本文提出分阶段视觉语言学习（SVLL）框架。该框架通过三阶段训练解耦空间定位与时间推理，并引入Bias-DPO对齐算法，通过注入专家轨迹偏置来解决标准DPO在物理约束下的幻觉问题。实验证明SVLL在AI2-THOR和真实机器人任务中表现优于GPT-4o等模型，显著提升了规划的物理接地性。
selection_source: fresh_fetch
motivation: 现有具身规划训练难以平衡空间定位与时间推理，且标准DPO因缺乏绝对似然约束容易产生不符合物理常识的幻觉行为。
method: 采用三阶段训练策略解耦时空推理，并设计Bias-DPO算法，通过显式最大化专家轨迹似然来增强模型对环境可供性的理解。
result: 在AI2-THOR基准和真实机器人实验中，SVLL的成功率超越了GPT-4o和Gemini-2.0-flash等顶尖模型，并大幅降低了物理约束违规率。
conclusion: 分阶段学习与带偏置的偏好优化相结合，是提升视觉语言模型在具身智能任务中鲁棒性和物理一致性的有效途径。
---
