...
motivation: 旨在解决高层VLM推理延迟高与底层VLA模型在复杂长程任务中语义深度不足之间的矛盾。
method: 构建了由VLM大脑、VLA小脑和视觉Critic组成的三系统架构，利用Critic进行动态任务调度、异常检测并集成人类启发式规则。
result: 在挑战性的长程操作基准测试中达到了SOTA性能，并在分布外（OOD）场景下表现出极强的鲁棒性。
conclusion: 该框架通过视觉引导的自适应调度，在最小化昂贵VLM调用的同时，实现了高效且稳健的机器人长程自主操作。
---

## 摘要
在视觉机器人操作中，平衡高层语义推理与底层反应式控制仍然是一个核心挑战。虽然视觉语言模型（VLM）擅长认知规划，但其推理延迟阻碍了实时执行。相反，快速的视觉-语言-动作（VLA）模型通常缺乏复杂长程任务所需的语义深度。为了弥补这一差距，我们引入了“闭环评论者”（Critic in the Loop），这是一种由动态 VLM-专家调度驱动的自适应分层框架。其核心是一个仿生三系统架构，包括用于全局推理的 VLM 大脑、用于反应式执行的 VLA 小脑以及一个轻量级的视觉评论者（Critic）。通过持续监控工作空间，评论者动态地路由控制权。它通过 VLA 维持常规子任务的快速闭环执行，并在检测到任务停滞或失败等执行异常时，自适应地触发 VLM 进行重新规划。此外，我们的架构无缝集成了受人类启发的规则，以直观地打破无限重试循环。这种基于视觉的调度最小化了昂贵的 VLM 查询，同时显著增强了系统在分布外（OOD）场景下的鲁棒性和自主性。在具有挑战性的长程操作基准测试上的全面实验表明，我们的方法达到了最先进的性能。
