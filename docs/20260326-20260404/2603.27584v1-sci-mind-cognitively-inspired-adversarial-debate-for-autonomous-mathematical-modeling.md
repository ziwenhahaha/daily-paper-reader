...
tldr: 针对大语言模型在数学建模中因缺乏领域基础和对抗验证而产生逻辑缺陷的问题，本文提出Sci-Mind框架。该框架模拟人类科学发现过程，通过经验记忆检索历史方案，并引入“理论家”与“实践者”的对抗辩论机制来优化模型，最后利用自验证策略确保代码执行。实验证明，Sci-Mind在建模严谨性和代码可执行性上显著优于现有自主代理。
selection_source: fresh_fetch
motivation: 现有LLM代理在数学建模时多依赖孤立推理，缺乏领域知识支撑和对抗性验证，导致生成的模型往往看似合理实则存在缺陷。
method: 提出Sci-Mind框架，集成经验记忆检索、理论与实践的对抗辩论机制以及基于形式谓词的自验证执行策略。
result: 在MM-Bench和EngiBench基准测试中，Sci-Mind在建模严谨性和代码可执行性方面均显著超越了领先的自主代理。
conclusion: 通过模拟人类科学发现中的协作与对抗机制，可以有效提升自主代理解决复杂数学建模问题的能力和可靠性。
---

## 摘要
现实世界的数学建模本质上是一种基于经验和协作的尝试。领域专家很少从零开始解决复杂问题；相反，他们借鉴历史案例进行类比，并让自己的假设接受严格的同行评审。然而，由大语言模型驱动的自主智能体主要依赖孤立的推理范式，由于缺乏领域基础和对抗性验证，经常生成看似合理但存在根本缺陷的模型。为了解决这些局限性，我们提出了 Sci-Mind，这是一个模拟人类科学发现过程的新型框架。Sci-Mind 集成了经验记忆检索（Experiential Memory Recall），用于检索可执行代码片段和建模范式描述符，将抽象推理建立在历史解决方案的基础上。随后，它采用了一种对抗性认知辩证法（Adversarial Cognitive Dialectic），其中优化数学连贯性的“理论家”（Theorist）与强制执行数据可行性的“实践者”（Pragmatist）通过竞争性目标进行辩论，以剔除优美但不可行的公式。自验证执行策略（Self-Validating Execution Strategy）在代码生成前通过形式谓词进一步确保蓝图的一致性，从而实现完全自主的执行。在 MM-Bench 和 EngiBench 基准测试上的广泛实验表明，Sci-Mind 在建模严谨性和代码可执行性方面均显著优于领先的自主智能体。
