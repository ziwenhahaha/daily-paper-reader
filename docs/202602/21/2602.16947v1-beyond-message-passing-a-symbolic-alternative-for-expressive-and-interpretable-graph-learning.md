...
tags: ["query:sr"]
score: 8.0
evidence: 用于可解释图学习的神经消息传递的符号替代方案
...
## 摘要
图神经网络 (GNN) 在药物发现等高风险领域已变得至关重要，但其黑盒性质仍是实现可信度的重大障碍。虽然自解释 GNN 试图弥补这一差距，但它们通常依赖于标准的消息传递骨干网络，这些网络继承了根本性的局限性，包括 1-Weisfeiler-Lehman (1-WL) 表达能力障碍和缺乏细粒度的可解释性。为了应对这些挑战，我们提出了 SymGraph，这是一个旨在超越这些约束的符号化框架。通过用离散结构哈希和基于拓扑角色的聚合取代连续消息传递，我们的架构在理论上超越了 1-WL 障碍，在没有微分优化开销的情况下实现了卓越的表达能力。广泛的实证评估表明，SymGraph 达到了最先进的性能，优于现有的自解释 GNN。值得注意的是，SymGraph 仅使用 CPU 执行即可在训练时间上实现 10 到 100 倍的加速。此外，与现有的基于规则的方法相比，SymGraph 生成的规则具有更优越的语义粒度，为科学发现和可解释人工智能提供了巨大潜力。
