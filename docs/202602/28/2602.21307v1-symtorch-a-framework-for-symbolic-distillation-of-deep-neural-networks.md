---
title: "SymTorch: A Framework for Symbolic Distillation of Deep Neural Networks"
title_zh: SymTorch：深度神经网络符号蒸馏框架
authors: "Elizabeth S. Z. Tan, Adil Soubki, Miles Cranmer"
date: 2026-02-24
pdf: "https://arxiv.org/pdf/2602.21307v1"
tags: ["query:sr2026-intent"]
score: 10.0
evidence: 使用 PySR 进行符号蒸馏的 SymTorch 库
tldr: "SymTorch 是一个旨在简化符号蒸馏（Symbolic Distillation）的库，它能将复杂的神经网络组件转化为可解释的数学公式。该框架通过自动化数据采集、处理 GPU-CPU 传输及集成 PySR 符号回归，解决了将符号化引入深度学习工作流的工程障碍。SymTorch 适用于 GNN、PINN 和 Transformer 等多种架构，并证明了通过将 LLM 的 MLP 层替换为符号代理，可在轻微性能损失下提升 8.3% 的推理吞吐量。"
motivation: 尽管符号蒸馏在发现物理定律和提高模型可解释性方面具有潜力，但其复杂的工程实现门槛限制了在深度学习工作流中的广泛应用。
method: SymTorch 通过封装神经网络组件、自动缓存输入输出数据并调用 PySR 引擎，实现了从神经计算到闭式数学表达式的自动化转换。
result: "实验表明该框架能有效处理多种模型架构，并在 LLM 推理加速实验中通过符号化 MLP 层实现了 8.3% 的吞吐量提升。"
conclusion: SymTorch 为深度学习与符号回归的结合提供了高效工具，降低了构建可解释、高性能符号代理模型的门槛。
---

## 摘要
符号蒸馏利用可解释的闭式数学表达式替换神经网络或其组件。这种方法在直接从训练好的深度学习模型中发现物理定律和数学关系方面展现出巨大潜力，但由于将符号回归集成到深度学习工作流中存在工程障碍，其应用仍然受限。我们推出了 SymTorch，这是一个通过封装神经网络组件、收集其输入输出行为，并利用 PySR 将其近似为人类可读方程来自动化该蒸馏过程的库。SymTorch 解决了阻碍应用的工程挑战，包括：GPU-CPU 数据传输、输入输出缓存、模型序列化，以及神经网络与符号前向传播之间的无缝切换。我们在包括图神经网络 (GNNs)、物理信息神经网络 (PINNs) 和 Transformer 模型在内的多种架构上演示了 SymTorch。最后，我们提出了一个通过用符号代理替换 MLP 层来加速大语言模型 (LLM) 推理的概念验证，在性能适度下降的情况下实现了 8.3% 的吞吐量提升。

## 速览
**TLDR**：本文介绍了 SymTorch，这是一个旨在简化深度神经网络符号蒸馏过程的框架。符号蒸馏通过将复杂的神经网络组件转化为可解释的数学表达式，有助于发现物理定律和数学关系。SymTorch 解决了 GPU-CPU 数据传输、缓存和模型序列化等工程障碍，实现了神经网络与符号化前向传播的无缝切换，并在 GNN、PINN 和 Transformer 等多种架构上证明了其有效性。 \
**Motivation**：尽管符号蒸馏在模型可解释性方面具有潜力，但由于将符号回归集成到深度学习工作流中存在较高的工程门槛，其应用受到了限制。 \
**Method**：SymTorch 通过包装神经网络组件并利用 PySR 自动收集输入输出行为，将其近似为人类可读的闭式数学方程。 \
**Result**：该框架成功应用于多种模型，并在 LLM 推理加速实验中通过符号替代 MLP 层实现了 8.3% 的吞吐量提升。 \
**Conclusion**：SymTorch 降低了符号蒸馏的技术门槛，为提高深度学习模型的可解释性和推理效率提供了一个实用的工具。

---

## Abstract
Symbolic distillation replaces neural networks, or components thereof, with interpretable, closed-form mathematical expressions. This approach has shown promise in discovering physical laws and mathematical relationships directly from trained deep learning models, yet adoption remains limited due to the engineering barrier of integrating symbolic regression into deep learning workflows. We introduce SymTorch, a library that automates this distillation by wrapping neural network components, collecting their input-output behavior, and approximating them with human-readable equations via PySR. SymTorch handles the engineering challenges that have hindered adoption: GPU-CPU data transfer, input-output caching, model serialization, and seamless switching between neural and symbolic forward passes. We demonstrate SymTorch across diverse architectures including GNNs, PINNs and transformer models. Finally, we present a proof-of-concept for accelerating LLM inference by replacing MLP layers with symbolic surrogates, achieving an 8.3\% throughput improvement with moderate performance degradation.