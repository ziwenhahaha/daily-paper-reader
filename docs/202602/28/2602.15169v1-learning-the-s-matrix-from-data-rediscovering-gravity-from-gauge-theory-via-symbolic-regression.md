...
## 速览
**TLDR**：本研究展示了利用符号回归等机器学习方法，直接从数值数据中自动重构散射振幅的解析结构。研究成功从杨-米尔斯振幅中重新发现了KLT关系（揭示了引力与规范场论的联系），并利用QR分解等特征选择技术识别了KK和BCJ关系，确定了振幅的最小基组。该方法在五点振幅下达到了极高精度，证明了数据驱动策略在探索物理理论隐藏关系中的潜力。 \
**Motivation**：探索机器学习（特别是符号回归）是否能自主发现散射振幅中复杂的解析结构和物理定律。 \
**Method**：采用符号回归处理杨-米尔斯振幅数据，并结合列主元QR分解进行特征选择以识别振幅基组。 \
**Result**：成功在五点振幅层面以高精度重新发现了KLT、KK和BCJ等关键物理关系，实现了从规范场论到引力的跨越。 \
**Conclusion**：符号回归是探索散射振幅解析结构的实用工具，为发现物理学中隐藏的数学关系提供了一种通用的数据驱动策略。
