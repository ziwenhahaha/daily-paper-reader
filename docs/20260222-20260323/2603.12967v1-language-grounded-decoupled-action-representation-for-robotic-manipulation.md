...
motivation: 旨在解决机器人操作中高层语义理解与底层精确控制难以对齐，导致在处理新任务时泛化性差的问题。
method: 提出LaDA框架，将动作解耦为平移、旋转和夹爪控制三个语义基元，并采用语义引导的对比学习与自适应加权策略。
result: 在LIBERO、MimicGen仿真环境及真实世界测试中，LaDA在任务成功率和对未见任务的泛化表现上均优于现有方法。
conclusion: 通过引入细粒度的解耦动作表示和语义对齐机制，LaDA为弥合机器人感知与控制之间的鸿沟提供了一种有效方案。
---
