...
tags: ["query:wam-vla"]
score: 9.0
evidence: VLA架构的插件式视觉注入
tldr: 针对视觉语言动作模型（VLA）在处理细粒度几何和时序信息方面的不足，本文提出了一种轻量级、即插即用的视觉注入模块 PVI。该模块通过零初始化的残差路径将辅助视觉特征（如时序视频特征）注入预训练的动作专家模型中，无需大幅修改架构即可显著提升多阶段复杂任务的性能，并在真实机器人实验中验证了其有效性。
selection_source: fresh_fetch
motivation: 现有的 VLA 模型由于预训练 VLM 的语义抽象特性，往往缺乏细粒度的几何线索和时序证据，且现有注入方法难以灵活处理时序信息。
method: 提出 PVI 模块，利用零初始化的残差路径将外部视觉表征（如 V-JEPA2 视频特征）注入预训练的动作专家，实现轻量级且编码器无关的增强。
result: 实验证明 PVI 显著优于基础策略，且时序视频特征在需要状态追踪的多阶段任务中表现优于静态图像特征，并在真实机器人双臂布料折叠任务中得到验证。
conclusion: PVI 为增强 VLA 模型的视觉感知提供了一种高效、通用的即插即用方案，证明了时序信息对于复杂机器人操作任务的重要性。
---
