...
tags: ["query:wam-vla"]
score: 9.0
evidence: 通过世界知识变化的先验引导VLA模型
tldr: 本研究针对现有视觉-语言-动作（VLA）模型仅关注预测未来绝对状态而忽视变化过程的问题，提出了ΔVLA框架。该框架通过建模相对于当前世界知识先验的变化量来指导动作生成，而非直接回归未来状态。其核心包含用于提取空间与语义线索的PWKE模块、量化知识演化的LWVQ潜空间以及减少干扰的CV-Atten注意力机制。实验证明，ΔVLA在模拟和真实机器人任务中均取得SOTA性能，且具备更高的效率。
selection_source: fresh_fetch
motivation: 现有VLA模型侧重于预测绝对的未来世界状态，缺乏对指导动作生成的关键世界知识变化过程的推理能力。
method: 提出ΔVLA框架，利用PWKE提取知识先验，通过LWVQ在离散潜空间建模知识变化，并采用CV-Atten实现解耦学习。
result: 在多个模拟基准测试和真实世界机器人任务中，ΔVLA均实现了最先进的性能并显著提升了运行效率。
conclusion: 通过建模世界知识的变化而非绝对状态，该研究为提升VLA模型的推理效率和动作生成质量提供了有效的新路径。
---
