...
## 摘要
主动感知与操作对于机器人与复杂场景的交互至关重要。现有方法难以将语义驱动的主动感知与鲁棒的、视点不变的执行统一起来。我们提出了 SaPaVe，这是一个以数据高效的方式联合学习这些能力的端到端框架。我们的方法将相机动作与操作动作解耦，而非将其置于共享的动作空间中，并遵循自底向上的训练策略：我们首先在大规模数据集上训练语义相机控制，然后使用混合数据联合优化这两类动作。为了支持该框架，我们引入了 ActiveViewPose-200K，这是一个包含 20 万个图像-语言-相机运动对的数据集，用于语义相机运动学习；此外还引入了一个 3D 几何感知模块，以提高动态视点下的执行鲁棒性。我们还提出了 ActiveManip-Bench，这是首个用于评估非固定视角设置下主动操作的基准测试。在仿真和真实环境中的广泛实验表明，SaPaVe 的性能优于 GR00T N1 和 π₀ 等近期的视觉-语言-动作模型，在真实世界任务中的成功率提升高达 31.25%。这些结果表明，通过解耦且协调的策略进行训练，紧密耦合的感知与执行能够实现高效且具泛化能力的主动操作。项目主页：https://lmzpai.github.io/SaPaVe
