...
## 速览
**TLDR**：针对机器人灵巧手操作中跨构型数据稀缺和训练成本高的问题，本文提出了XL-VLA框架。该框架通过构建一个跨手部构型的统一潜动作空间，使视觉-语言-动作（VLA）模型能够无缝集成并利用不同机器手的演示数据。实验证明，该方法在跨构型学习和操作性能上优于传统的原始关节空间模型，为实现可扩展的灵巧操作提供了有效方案。 \
**Motivation**：随着新型灵巧手不断涌现，为每种构型单独收集大规模训练数据成本极高，亟需一种可扩展的跨构型学习方法。 \
**Method**：提出XL-VLA框架，通过引入一个与具体构型无关的统一潜动作空间，将其直接嵌入标准VLA架构中以实现跨手部训练。 \
**Result**：实验结果显示，XL-VLA在多项任务中均优于直接在原始关节空间操作的基准VLA模型。 \
**Conclusion**：统一的潜动作空间是实现高效、可扩展的跨构型机器人灵巧操作的关键，能有效复用异构数据。
