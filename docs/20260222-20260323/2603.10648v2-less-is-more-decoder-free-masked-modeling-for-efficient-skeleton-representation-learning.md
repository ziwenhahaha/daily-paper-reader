...
## 摘要
基于骨骼的动作表示学习领域已从对比学习 (CL) 演进到掩码自编码器 (MAE) 架构。然而，每种范式都面临固有的局限性：CL 往往忽略细粒度的局部细节，而 MAE 则受困于计算量巨大的解码器。此外，MAE 存在严重的计算不对称性——虽然在预训练期间受益于高效的掩码机制，但在下游任务中却需要对全序列进行详尽处理。为了解决这些瓶颈，我们提出了 SLiM (Skeleton Less is More)，这是一个通过共享编码器将掩码建模与对比学习相统一的新型框架。通过舍弃重构解码器，SLiM 不仅消除了计算冗余，还迫使编码器直接捕获判别性特征。SLiM 是首个采用无解码器掩码建模进行表示学习的框架。关键在于，为了防止由高度骨骼-时间相关性引起的平凡重构，我们引入了语义管道掩码，以及旨在确保不同时间粒度下解剖结构一致性的骨骼感知增强。大量实验表明，SLiM 在所有下游协议中始终保持着最先进的性能。值得注意的是，我们的方法在提供卓越准确性的同时具有极高的效率，与现有的 MAE 方法相比，推理计算成本降低了 7.89 倍。
