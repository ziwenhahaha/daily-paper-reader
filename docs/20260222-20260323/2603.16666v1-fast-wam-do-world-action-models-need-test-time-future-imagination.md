...
tags: ["query:wam-vla"]
score: 10.0
evidence: Fast-WAM：用于具身控制的世界动作模型
tldr: Fast-WAM 探讨了世界动作模型（WAM）在推理时是否需要显式的未来想象。研究发现，虽然现有 WAM 依赖耗时的视频去噪生成未来图像，但其核心优势其实源于训练阶段的视频建模。Fast-WAM 保留了视频协同训练，但在推理时跳过预测步骤，在 LIBERO 和 RoboTwin 等基准测试中表现优异，且速度提升了 4 倍以上，实现了 190ms 的实时控制。
selection_source: fresh_fetch
motivation: 现有的 WAM 模型因推理时需要迭代生成未来视频而导致高延迟，作者旨在探究显式的未来想象在测试阶段是否真正必要。
method: 提出 Fast-WAM 架构，在训练时保留视频协同训练以增强表征，但在推理时直接输出动作而跳过耗时的视频生成过程。
result: Fast-WAM 在仿真和真实世界任务中达到了 SOTA 水平，推理延迟仅为 190ms，比传统的“先想象后执行”模型快 4 倍以上。
conclusion: 研究表明 WAM 的主要价值在于训练期间通过视频预测提升世界表征能力，而非在推理时生成未来观测结果。
---
