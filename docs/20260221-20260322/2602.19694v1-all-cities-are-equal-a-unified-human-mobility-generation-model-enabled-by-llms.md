...
tldr: "本研究针对现有移动生成模型在数据匮乏城市表现不佳的问题，提出了统一模型UniMob。该模型创新性地结合了大语言模型（LLM）生成的语义旅行计划、统一空间嵌入模块以及基于扩散模型的移动生成器，实现了跨城市的通用移动轨迹生成。实验证明，UniMob在多城市数据集上比现有方法提升了30%以上，且在零样本和少样本场景下表现稳健，为智慧城市建设提供了公平且隐私保护的数据支持。"
selection_source: fresh_fetch
motivation: 现有的人类移动生成方法高度依赖特定城市的大规模数据，导致在数据资源有限的城市中效果显著下降，无法实现跨城市的公平应用。
method: 提出UniMob模型，利用LLM制定高层旅行计划，通过统一空间嵌入模块对齐不同城市表征，并结合扩散模型捕捉复杂的时空移动特征。
result: "在五个城市的真实数据集测试中，UniMob在多项评估指标上均优于现有基准模型30%以上，并证明了其在零样本学习和隐私保护方面的优势。"
conclusion: UniMob成功构建了一个跨城市的统一移动生成框架，有效解决了城市间的数据不平等问题，为下游智慧城市任务提供了高质量的合成数据。
---

## 摘要
合成人类移动性生成作为一种符合伦理且实用的方法，在支持智能城市系统的数据需求方面正受到越来越多的关注。现有方法主要在数据丰富的城市中表现良好，而在数据资源有限的城市中，其有效性显著下降。然而，生成可靠的人类移动性数据的能力不应取决于城市的规模或可用资源，所有城市都应得到平等的对待。为了解决这一开放性问题，我们提出了 UniMob，一种跨城市的统一人类移动性生成模型。UniMob 由三个主要组件组成：一个由大语言模型（LLM）驱动的出行规划器，用于推导高层级、具有时间感知能力且富有语义信息的出行计划；一个统一的空间嵌入模块，将不同城市的空间区域投影到一个共享的表示空间中；以及一个基于扩散模型的移动性生成器，在推导出的出行计划指导下，捕捉人类运动的联合时空特征。我们使用涵盖五个城市的两个真实世界数据集对 UniMob 进行了广泛评估。综合实验表明，UniMob 显著优于最先进的基准模型，在多个评估指标上实现了超过 30% 的提升。进一步的分析证明了 UniMob 在零样本和少样本场景下的鲁棒性，强调了 LLM 指导的重要性，验证了其隐私保护特性，并展示了其在下游任务中的适用性。
