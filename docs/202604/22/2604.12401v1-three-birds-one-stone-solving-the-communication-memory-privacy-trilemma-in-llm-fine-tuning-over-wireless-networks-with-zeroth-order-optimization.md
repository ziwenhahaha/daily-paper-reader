...
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 通过零阶优化实现通信高效的联邦LLM微调
tldr: 在无线网络中联邦微调大语言模型（LLM）面临通信、内存和隐私的三重困境。本文提出了pAirZero框架，将零阶（ZO）优化与空中计算（OTA）技术协同结合。该框架使资源受限的边缘设备仅需比特级通信开销和推理级内存成本即可参与微调，同时通过优化模型自适应调整传输功率和噪声以确保隐私安全。实验表明，该方法在大幅降低通信负载和内存消耗的同时，有效保障了数据隐私。
source: arxiv
...
motivation: 在无线边缘网络中联邦微调大语言模型时，面临着高维梯度带来的通信和内存开销巨大，以及本地梯度易泄露用户隐私的三重挑战。
...
conclusion: pAirZero框架成功打破了无线网络中LLM微调的通信、内存与隐私三重困境，实现了安全高效的联邦微调。
---
