...
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 异步概率聚合降低联邦学习通信开销
tldr: 针对灾害决策支持系统中网络延迟和精度不足的问题，传统联邦学习（FL）存在通信成本高且难以在异构CNN架构间同步的局限。本文提出了一种基于异步概率聚合和反馈蒸馏的去中心化集成框架。该方法通过交换类别概率向量替代模型权重，在保障隐私的同时大幅降低了通信开销，使多样化的CNN模型能够异步协作，为实时灾害响应提供了一种高精度、可扩展且资源高效的解决方案。
source: arxiv
...
motivation: 传统联邦学习在异构卷积神经网络架构中面临高通信成本和严格同步要求的限制，阻碍了灾害决策支持系统的快速准确响应。
method: 提出一种去中心化集成框架，通过异步概率聚合和反馈蒸馏，将模型间的交换单元从权重替换为类别概率向量。
result: 实验表明，该方法不仅大幅降低了通信需求，还在灾害图像识别任务上的表现优于传统独立模型和标准联邦学习方法。
conclusion: 该框架成功实现了异构模型的异步协作，为资源受限环境下的实时灾害响应提供了一种高精度、可扩展的解决方案。
---
