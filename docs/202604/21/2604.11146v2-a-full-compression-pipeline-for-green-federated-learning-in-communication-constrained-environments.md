...
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 面向联邦学习的全压缩流水线以减少通信开销
tldr: 在通信受限的环境中，联邦学习（FL）常面临严重的通信与计算开销。本文提出了一种全压缩流水线（FCP），将剪枝、量化和霍夫曼编码三种深度压缩技术整合到统一的端到端框架中。该方法在保持模型竞争力的同时大幅降低了传输成本和资源消耗，为构建绿色可持续的联邦学习系统提供了高效解决方案。
source: arxiv
...
method: 提出全压缩流水线（FCP），结合剪枝、量化和霍夫曼编码技术，并引入统一评估框架来综合衡量通信与计算成本。
result: "在CIFAR-10数据集的代表性场景中，FCP实现了超过11倍的模型大小缩减和60%以上的训练加速，且准确率仅下降2%。"
conclusion: FCP能够在通信受限环境中显著降低联邦学习的资源消耗并大幅提升训练效率，同时有效维持模型准确率。
---
