...
tldr: 在去中心化大规模机器学习中，通信压缩等操作常导致梯度估计存在偏差，并在数据异构性下严重影响模型收敛。为此，本文提出了一种名为Biased-DMT的新型去中心化优化算法。该算法能够有效解耦网络拓扑与数据异构性的影响，在非凸环境中实现了随节点数量增加的线性加速，并在面对绝对偏差时能消除异构误差，显著提升了系统的收敛性能与鲁棒性。
source: arxiv
...
motivation: 现有的去中心化随机优化方法在实际应用中常受限于有偏梯度估计（如通信压缩引起），导致在数据异构场景下收敛性能严重下降。
method: 提出了一种新型去中心化算法Biased-DMT（带有偏梯度的去中心化动量跟踪算法），以在有偏梯度信息下实现可靠的优化。
result: 理论证明该算法在非凸环境中实现了线性加速，成功解耦了网络拓扑与数据异构性的影响，并在绝对偏差下收敛至精确的物理误差下限。
conclusion: Biased-DMT算法有效解决了有偏梯度下的去中心化优化难题，理论分析与实验均证实了其在多种去中心化学习场景中的鲁棒性与有效性。
---
