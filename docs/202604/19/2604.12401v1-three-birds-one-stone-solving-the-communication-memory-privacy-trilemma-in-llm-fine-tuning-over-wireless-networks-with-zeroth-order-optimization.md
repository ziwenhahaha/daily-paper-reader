...
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 基于零阶优化的通信高效分布式训练
tldr: 在无线网络中联邦微调大语言模型（LLM）面临通信开销大、内存要求高和隐私泄露的三重困境。本文提出了pAirZero框架，将零阶（ZO）优化与空中计算（OTA）技术协同结合。该框架使资源受限的边缘设备仅需比特级通信负载和推理级内存成本即可参与LLM微调，同时通过优化模型自适应调整传输功率和噪声以确保隐私安全，成功解决了上述三难困境。
source: arxiv
...
motivation: 在无线边缘网络中联邦微调大语言模型时，面临着高维梯度交换带来的巨大通信与内存开销，以及本地梯度导致的隐私泄露风险。
method: 提出pAirZero框架，结合零阶优化与空中计算技术，并通过严格的优化模型自适应调整传输功率与噪声水平以保障隐私。
result: "实验表明，该方法在OPT-125M模型上仅需25%的峰值内存成本，通信负载比传统方法降低了数个数量级，并实现了安全的LLM微调。"
conclusion: pAirZero框架成功打破了无线网络中LLM微调的通信、内存与隐私三重困境，实现了高效且安全的分布式模型训练。
---
