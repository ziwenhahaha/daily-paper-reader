...
tags: ["query:dgnn-nas"]
score: 7.0
evidence: 面向通信高效分布式训练的全压缩流水线
tldr: 在通信受限的环境中，联邦学习（FL）常面临严重的通信与计算开销。本文提出了一种面向绿色联邦学习的全压缩流水线（FCP），将剪枝、量化和霍夫曼编码三种深度压缩技术整合到统一的端到端框架中。该方法在保持模型竞争力的同时大幅降低了传输成本和资源消耗，为资源受限场景下的可持续FL提供了高效解决方案。
source: arxiv
...
motivation: 联邦学习在分布式训练中存在显著的通信和计算开销，限制了其可扩展性与可持续性。
method: 提出全压缩流水线（FCP），将剪枝、量化和霍夫曼编码集成于端到端框架，并构建统一评估框架衡量通信与计算开销。
result: "在CIFAR-10数据集的代表性场景中，FCP实现了超过11倍的模型大小缩减和60%以上的训练加速，且准确率仅下降2%。"
conclusion: FCP能够在通信受限环境中显著降低联邦学习的资源消耗并大幅提升训练效率，同时有效维持模型精度。
---
