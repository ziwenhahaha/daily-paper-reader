...
tags: ["query:dgnn-nas"]
score: 6.0
evidence: 带通信压缩和偏置梯度的去中心化随机优化
tldr: 在去中心化大规模机器学习中，通信压缩等操作常导致梯度估计存在偏差，并在数据异构性下严重影响模型收敛。为此，本文提出了一种名为Biased-DMT的新型去中心化优化算法。该算法能够有效解耦网络拓扑与数据异构性的影响，在非凸环境中实现线性加速，并在绝对偏差下消除异构误差，显著提升了系统的收敛性能与鲁棒性。
source: arxiv
...
motivation: 去中心化优化中广泛使用的有偏梯度估计器在数据异构场景下会严重破坏模型的收敛性能。
method: 提出了一种新型去中心化算法Biased-DMT，旨在有偏梯度信息下实现可靠的优化并解耦网络与数据异构性的影响。
result: 理论证明该算法在非凸环境中实现了节点数的线性加速，并在绝对偏差下收敛至精确的物理误差底限。
conclusion: Biased-DMT算法有效克服了有偏梯度和数据异构性带来的挑战，在各类去中心化学习场景中展现出卓越的鲁棒性与实用性。
---
